...
export const THREAT_MODEL: Threat[] = [
  ...
  // ---------------------------------------------------------------------
  {
    ...
    defence:
      "Per-context, and no longer uniform -- the extension-wide `connect-src 'self'` claim this entry used to make stopped being true when the GitHub SSH-key lookup shipped. PANEL AND WELCOME REALMS: still exactly `connect-src 'self'`, now via the `<meta http-equiv=\"Content-Security-Policy\">` tag in `sidepanel.html` rather than via the manifest. Meta CSP can only tighten, never loosen, and the browser enforces it. WORKER REALM: the manifest CSP is `connect-src 'self' https://api.github.com/users/ https://keys.openpgp.org/vks/v1/` -- two origins, each narrowed to one path prefix, GET only, and neither reachable by any code path unless the user has `keyDiscoveryEnabled` on (the manifest cannot narrow with a setting; the CODE PATH is what the preference removes). Defence in depth in both realms: `network-lockdown.ts` freezes `fetch` non-configurable (forcing `credentials: \"omit\"`, stripping Cookie/Authorization/X-Api-Key, rejecting plain HTTP and POST/PUT/PATCH/DELETE) and stubs XHR / WebSocket / EventSource / RTCPeerConnection / sendBeacon. Re-asserted at build time per context: the audit pins the worker bundle to exactly two `fetch` call sites, one resolving to each permitted origin, and to exactly those two `https://` origin literals, and the page bundles to the two wasm loaders and a fixed set of non-connect URL literals.",
    status: "partial",
    rationale:
      "Both layers are static or same-realm. The build-time audit proves no unexpected network code ships; it does not prove the runtime stubs cannot be circumvented in a realm we do not control. A runtime assertion suite for the lockdown does not exist yet. The CSP half is browser-enforced and so does not share that weakness -- but it is now only as narrow as `https://api.github.com/users/` plus `https://keys.openpgp.org/vks/v1/` in the worker, and CSP does not path-match query strings, which is T-GITHUB-CSP-SCOPE. The audit's URL-literal checks are a change detector, not a proof: a destination assembled at runtime from fragments never appears as a literal.",
  ...
  {
    id: "T-KEYSERVER-LOOKUP-DISCLOSURE",
    title:
      "The keyserver lookup tells keys.openpgp.org who the user is about to write to",
    attacker:
      "keys.openpgp.org itself, and anyone with access to its request logs or to the network path (the user's ISP, employer, or a state observer of TLS metadata). No compromise of anything is required -- this is the ordinary operation of the feature.",
    defence:
      "A preference, and nothing else. Asking for `/vks/v1/by-email/<addr>` IS the feature; the request discloses the identity being looked up, the requester's IP, and the time, before any message exists. `keyDiscoveryEnabled` (on by default, OFF in the strictest preset) removes the only code path that issues it.",
    status: "accepted",
    rationale:
      'The twin of T-GITHUB-LOOKUP-DISCLOSURE, and WORSE THAN IT in one specific way that is the reason this is a separate entry: a GitHub lookup discloses an account name, while this discloses an EMAIL ADDRESS -- the thing the user is about to correspond with, in the clear in a URL path, to a third party who has no other reason to know it. An account name is a pseudonym; an address usually is not. WHAT IS SENT: the canonical query (a lowercased address, or an uppercase fingerprint) in the URL path, over TLS. Nothing else -- no message content, no key material, no vault identifier, no extension id, no installation id, no telemetry, and no body (the request is a GET). WHAT IT IS NOT JOINED TO: `network-lockdown.ts` forces `credentials: "omit"` and strips `Cookie` and `Authorization`, and no host permission exists (the endpoint answers unauthenticated CORS with `access-control-allow-origin: *`, measured), so the request carries no session. As with GitHub that is narrower than "anonymous" and must not be read as one -- it can be joined on IP address. LOOKING UP BY FINGERPRINT DISCLOSES LESS and is a real mitigation the UI offers rather than merely permits: `/vks/v1/by-fingerprint/<FP>` names a key, not a person, and the same field accepts it. Say so to a user who cares. WHEN IT FIRES: only on an explicit button press in the import UI. Never on a timer, never as a background refresh, never on encrypt, never on unlock, never on install, and never at all while `keyDiscoveryEnabled` is off. One press, one request. THE ALTERNATIVE STAYS FIRST-CLASS: pasting the armored cert does exactly the same thing with no network at all, and it is the same code path once the armor exists (`prepareImport`) -- the lookup is a convenience that fetches bytes the user could have pasted. Accepted rather than partial because there is no partial version: keys.openpgp.org has no private-information-retrieval mode, an anonymising proxy would only move the disclosure to whoever runs the proxy, and this project does not run servers.',
    section: "§7, §13",
  },
  {
    id: "T-KEYSERVER-KEY-SUBSTITUTION",
    title: "keys.openpgp.org asserts a key; nothing proves it is the right one",
    attacker:
      "keys.openpgp.org (or an insider there); anyone who can complete the service's email round-trip for the target's address, which means anyone who controls that mailbox or its delivery path; anyone who can mint a publicly-trusted certificate for keys.openpgp.org and sit on the path.",
    defence:
      "None cryptographic. A VKS answer is an ASSERTION BY THE SERVICE, not a proof of anything. There is no transparency log, no signature over the response the client checks, no certificate pinning, and no out-of-band confirmation. The import preview shows the fingerprint and User IDs before anything is stored, and the resulting contact records `source` so the UI can say where the key came from.",
    status: "accepted",
    rationale:
      "The twin of T-GITHUB-KEY-SUBSTITUTION, and the differences are worth stating rather than folding into it. WHAT THE SERVICE'S VERIFICATION ACTUALLY MEANS: keys.openpgp.org only publishes an address after a round-trip email to it, so a by-email answer does carry a claim GitHub's does not -- \"whoever controlled this mailbox at upload time held this key\". THAT IS MAILBOX CONTROL, NOT IDENTITY, and it is exactly the property an attacker who has compromised the mailbox, or who can intercept mail to it, also has. It is a weaker claim than it feels like and must not be presented as verification. WHAT A FINGERPRINT LOOKUP CHANGES: by-fingerprint is a lookup by content, so a substituted answer cannot match the fingerprint the user asked for -- the query is the check, and this is the one path in the whole feature where the answer is self-verifying. It only helps a user who already has the fingerprint from somewhere else, which is the same limitation the preview has. THE PREVIEW IS NOT VERIFICATION, for the same reason as the GitHub entry: for the common case -- looking someone up precisely because you do NOT have their key -- it is a display of a number with nothing to compare it against. Unlike GitHub's stripped SSH lines, a cert carries User IDs, which LOOK like an identity claim and are not one: they are strings the key's creator typed. WHAT `source` DOES AND DOES NOT BUY is unchanged: a provenance label, not a trust decision, with no affordance to record an out-of-band verification and upgrade it. Accepted, not pending: the fixes are TOFU pinning with change alerts or a real web of trust, and both are features this app has deliberately not taken on. Distinct from T-KEYSERVER-UNTRUSTED-PARSE, which is about safely handling the BYTES.",
    section: "§7, §13",
  },
  {
    id: "T-KEYSERVER-UNTRUSTED-PARSE",
    title: "Attacker-controlled certificate bytes parsed in the service worker",
    attacker:
      "Anyone who controls what keys.openpgp.org returns: the service itself, a TLS interceptor returning an arbitrary body, or a captive portal or proxy substituting HTML for a certificate.",
    defence:
      "A deliberately boring parse, and the same shape as the GitHub one. In the WORKER: hard caps applied before the body is in memory, not merely before the parse -- a 15s request deadline, a `Content-Length` pre-check that cancels without reading, and a bounded reader (`lib/net/capped-body.ts`, shared with the GitHub lookup) that stops at 128 KiB measured in BYTES. Content type must be `application/pgp-keys`. Then armor-marker extraction only: the text between a matched BEGIN/END PUBLIC KEY pair, first block forwarded, any extras counted and surfaced. No `JSON.parse`, no eval, no dynamic import, no Function constructor. Error text from the response never crosses the message boundary -- results are tagged codes, not prose the service wrote, which matters more here than for GitHub because its 404 body is `text/html` quoting the query back. In the PANEL: the armor goes through `prepareImport`, the identical wasm path a pasted cert takes.",
    status: "defended",
    rationale:
      "Recorded separately from T-GITHUB-UNTRUSTED-PARSE because the bytes are a different shape (armor, not JSON) and the cap is a different size, so the two make different promises even though the invariant behind them is the same one. THE INVARIANT THAT KEEPS THE WORKER BORING: the worker is a transport that shape-checks; the wasm engine is the only thing that decides validity. The worker NEVER concludes that a string is a key -- it forwards armor, and the panel re-parses it through `prepareImport`, the identical path a paste takes, before it can become a contact. A PRIVATE KEY BLOCK cannot be forwarded at all: the extractor matches PUBLIC armor only, so the endpoint has no way to hand this app secret-shaped material to classify. WHAT THIS DOES NOT CLAIM: nothing here says the key is TRUSTWORTHY -- that is T-KEYSERVER-KEY-SUBSTITUTION, and the two entries deliberately do not overlap.",
    verifiedBy: [
      "apps/pgp/lib/keyserver/response.test.ts",
      "apps/pgp/lib/keyserver/query.test.ts",
  ...
  },
  {
    ...
    defence:
      "Bounded, not eliminated. The app now BOTH reads and writes these messages, so it does choose parameters -- and the choices are the strongest the format allows: AES-256 pinned explicitly, and Sequoia's S2K default of SHA-256 iterated at count 0x3e00000, which is the LARGEST the OpenPGP wire format can encode. An 8-character minimum matches the vault's own. None of that changes what a shared password is worth against whoever ends up holding it.",
    status: "accepted",
    rationale:
      "Recorded because the feature reads like key-based decryption in the UI -- same workspace, same prompt row, same result pane -- and is a categorically weaker thing. WHAT IT IS NOT: there is no identity, no key, and no forward secrecy. A public-key message is readable by the holder of a private key that never travelled; a symmetric message is readable by anyone holding a shared secret that necessarily did travel, over a channel this app has no view of. Whoever the sender gave the password to, and whoever intercepted it in transit, can read the message -- and can read it again later, from a copy of the ciphertext, because nothing about the message expires. THE STRENGTH IS THE SENDER'S CHOICE ENTIRELY: GnuPG's S2K iteration count is the only work factor between a captured message and an offline dictionary attack, and it is baked into the ciphertext by whoever ran `gpg -c`. This app reads that parameter; it does not and cannot raise it. ON SHIPPING ENCRYPTION, having first declined to: the objection was that creating such a message means choosing its parameters on the user's behalf. That objection turned out to be weaker than it sounded once the numbers were checked -- the S2K count available IS the format's maximum, so there is no weaker-vs-stronger judgement call to get wrong, only the question of whether to inherit it silently or pin and test it. It is pinned and tested. What remains true is everything above: the parameters bound the COST of an offline attack, they do not change the fact that the secret travelled. A SIGNATURE IS STILL WORTH SOMETHING: a symmetric message can be signed, and the signature is checked on this path exactly as on the key path, so `signatureStatus: valid` against a known signer means the same thing here as anywhere. That is the one authenticity claim available, and it comes from the signature, never from the password.",
    section: "§7, §2",
  },
  {
  ...
  },
  {
    ...
    defence:
      "The tab is a dumb view with no vault, keys, wasm, storage or network. The panel keeps the message sealed under its in-WASM draft key and posts it over a nonce-named runtime port, to every tab connected on that nonce (a duplicate included), only while the vault is open. `doMasterLock` calls `lockNow` first and synchronously: every tab gets `locked`, clears its text and drops the signer card, and no in-flight post can follow; unlock resends. The last tab closing zeroes and drops the sealed copy; closing the panel disconnects the ports (the tabs clear and close) and the draft key dies with the page. The signer is sent as display fields only. Only a nonce is in the URL and nothing is written to storage. Every wipe also clears V8's last-RegExp-match subject (`forgetLastRegExpMatch`).",
    status: "accepted",
    rationale:
