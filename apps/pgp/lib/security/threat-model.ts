...
export const THREAT_MODEL: Threat[] = [
  ...
  },
  {
    ...
    status: "accepted",
    rationale:
      "CONFIRMED EXPLOITABLE, not theoretical. The wasm-bindgen glue ships as an ES module chunk whose hashed filename appears verbatim in the readable entry chunk; the ES module registry is keyed by resolved URL, so import()-ing it returns the ALREADY-INITIALISED instance with the live KEY_STORE rather than a fresh empty one. In-page JS doing import() + getKeyArmored(1) returns plaintext secret key material with no user gesture, no CDP and no debugger. Sequential handles from 1 make the sweep trivial but are not the root cause -- randomising them would not fix this, since the attacker can also read the handle off the React tree or the decoder path. Accepted rather than pending because it is not fixable with in-realm mechanisms: the only structural fix is moving the engine into a Worker and giving the panel a narrow MessagePort. What limits it: no handle survives a lock (§6), so the attack window is exactly the unlocked window, and §7 still constrains exfiltration. Documented in §8.10.",
  ...
  },
  {
    ...
    defence:
      "Partial. `network-lockdown.ts` now freezes `TextEncoder.prototype.encode`/`encodeInto`, `TextDecoder.prototype.decode`, `Crypto.prototype.getRandomValues` and `Clipboard.prototype.writeText` non-writable and non-configurable at first import.",
    status: "partial",
    rationale:
  ...
  },
  {
    ...
    defence:
      "Partial. `Crypto.prototype.getRandomValues` is now frozen non-writable at first import (see T-PRIMITIVE-HOOK). The WASM module's only entropy source remains a per-call `globalThis.crypto.getRandomValues` lookup in the wasm-bindgen glue.",
    status: "partial",
    rationale:
      "The most severe variant of T-PRIMITIVE-HOOK and the one with the longest blast radius: it does not exfiltrate an existing key, it makes every key generated afterwards predictable, silently, with valid-looking output and no artefact left in the vault to find later. Confirmed reachable before the freeze (hook installed, call count observed). Was entirely absent from the threat model before this audit. The freeze does not help against a dependency earlier in the module graph, and entropy is the one case where that residual risk is unbounded in time -- a poisoned key stays poisoned. Worth doing independently: mix WASM-side entropy into keygen so a compromised JS RNG alone cannot determine the output. Not yet implemented.",
    verifiedBy: ["apps/pgp/e2e/hostile-dep.spec.ts"],
    section: "§8.10",
  },
  {
