 ...
 *  (and including) the final Import click: pick the file -> preview ->
 *  unlock + re-protect with `PASSWORD`. Assertions stay in the tests. */
async function driveProtectedImport(
  ...
  // Unlock and re-protect are one step: the key announces its own
  // passphrase protection above the protection picker.
  await expect(
...
test("rejects a wrong source passphrase with a clear error", async ({
  ...
  await expect(
    panel.getByRole("button", { name: "Import", exact: true }),
  ).toBeVisible();
});
...
test("surfaces a legible error for an offline-primary key (stubbed secret)", async ({
  ...
  await expect(panel.getByText(/offline primary key/).first()).toBeVisible();
